    ...
    <div class="flex flex-1 flex-col">
        <header class="p-6">
            <RouterLink :to="{ name: 'landing' }"
                class="inline-flex items-center gap-2 text-sm font-bold text-gray-x-light transition duration-200 ease-in-out hover:text-gray-2x-light">
        ...
        <main class="flex flex-1 items-center justify-center px-6 py-4">
          ...
          -->
                <div
                    class="flex flex-col gap-6 rounded-[1.75rem] border-2 border-gray-light bg-gray-x-dark/85 p-8 shadow-2xl shadow-black/40 backdrop-blur-xl">
                    ...
                    <div class="relative grid grid-cols-2 rounded-xl border-2 border-gray-light bg-gray-dark/60 p-1">
                        <div class="absolute inset-y-1 w-[calc(50%-0.25rem)] rounded-lg bg-teal-light transition-transform duration-300 ease-out"
                    ...
                    <form class="flex flex-col gap-4" @submit.prevent="submit">
                        <div class="flex flex-col gap-2">
                            ...
                            <input id="identifier" ref="identifierEl" v-model="identifier" :class="fieldClass"
                                type="text" :autocomplete="isRegister ? 'username' : 'username email'"
                                :placeholder="isRegister ? 'user' : 'user or user@example.com'" spellcheck="false"
                                autocapitalize="none" />
                            <Transition name="hint">
                                <p v-if="identifierError" class="text-xs font-bold text-amber-400">
                                    {{ identifierError }}
                                </p>
                            </Transition>
                        </div>
                            ...
                            <div v-if="isRegister" class="flex flex-col gap-4 overflow-hidden">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <input id="email" v-model="email" :class="fieldClass" type="email"
                                        autocomplete="email" placeholder="user@example.com" spellcheck="false"
                                        autocapitalize="none" />
                                    <Transition name="hint">
                                        <p v-if="emailError" class="text-xs font-bold text-amber-400">
                                            {{ emailError }}
                                        </p>
                                    </Transition>
                                </div>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                <input id="password" v-model="password" :class="[fieldClass, 'pr-14']"
                                ...
                                <button type="button" tabindex="-1"
                                    ...
                                    class="absolute right-2 top-1/2 flex h-9 w-10 -translate-y-1/2 cursor-pointer items-center justify-center rounded-lg text-gray-x-light transition duration-200 ease-in-out hover:bg-gray-light/40 hover:text-gray-2x-light">
                                    <div class="h-full w-5" :style="{
                                        backgroundColor: `var(--color-teal-light)`,
                                        mask: `url(/${showPassword ? 'hidden.png' : 'eye.png'}) no-repeat center / contain`,
                                        '-webkit-mask': `url(/${showPassword ? 'hidden.png' : 'eye.png'}) no-repeat center / contain`,
                                    }"></div>
                                </button>
                            </div>

                            <Transition name="hint">
                                <p v-if="passwordError" class="text-xs font-bold text-amber-400">
                                    {{ passwordError }}
                                </p>
                            </Transition>
                     ...
                     change what the user types -->
                            <div v-if="isRegister && password" class="flex items-center gap-3 pt-1">
                                <div class="flex flex-1 gap-1">
                                    <span v-for="i in 4" :key="i"
                                        ...
                                        :class="i <= strength ? strengthMeta.class : 'bg-gray-light'"></span>
                                </div>
                        ...
                        <button type="submit" :disabled="!canSubmit"
                            class="group relative mt-1 w-full cursor-pointer overflow-hidden rounded-xl border-2 border-teal-light bg-teal-light py-3.5 font-bold text-gray-dark transition duration-200 ease-in-out hover:brightness-110 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:brightness-100 focus-visible:outline-2 focus-visible:outline-teal-light focus-visible:outline-offset-2">
