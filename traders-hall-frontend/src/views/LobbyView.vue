...
const { myGames, loadingMine, hasLoadedMine, busy, error } = storeToRefs(games)
...
async function poll() {
  ...
  inFlight = true
  try {
    await games.fetchMine({ silent: true })
  } finally {
    inFlight = false
  }
}
...
function deleteWarning(game) {
  ...
  if (game.players.length > 1)
    return `Removes the table for ${game.players.length - 1} other player(s).`
...
const STATUS_META = {
  ...
  in_progress: {
    label: 'In progress',
    class: 'border-emerald-400/50 bg-emerald-400/15 text-emerald-400',
  },
}

const panelClass =
  'flex flex-1 flex-col gap-4 rounded-[1.5rem] border-2 border-gray-light bg-gray-x-dark/85 p-6 ' +
  'shadow-2xl shadow-black/30 backdrop-blur-xl justify-between'
...
<template>
  <div class="mx-auto flex w-full max-w-6xl min-h-0 flex-col gap-4 p-6">
    <header
      class="grid shrink-0 items-center grid-cols-3 rounded-[1.5rem] border-2 border-gray-light bg-gray-x-dark/85 p-4 backdrop-blur-xl"
    >
      <h1 class="text-2xl font-bold tracking-widest text-gray-2x-light">TRADERS HALL</h1>
      <IdeaByMesum semi-compact class="-mb-4 -mt-2" />
      <div class="flex items-center gap-3 justify-self-end">
        ...
        <button
          ...
          title="Log out"
          @click="logout"
          class="flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border-2 border-gray-light bg-gray-dark text-gray-x-light transition duration-200 ease-in-out hover:border-rose-400 hover:text-rose-400"
        >
          ⏻
        </button>
      </div>
    ...
    <div class="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
          ...
          <template v-else>
            <div class="flex flex-col gap-2">
              ...
              <div class="flex items-center gap-3">
                <div
                  class="min-w-0 flex-1 rounded-xl border-2 border-teal-light/50 bg-gray-dark px-4 py-3 text-center text-3xl font-bold tracking-[0.3em] text-teal-light tabular-nums"
                >
                  {{ created.joinCode }}
                </div>
                <button
                  type="button"
                  @click="copyCode"
                  class="h-14 w-20 shrink-0 cursor-pointer rounded-xl border-2 border-gray-light font-bold text-gray-x-light transition duration-200 ease-in-out hover:border-gray-x-light hover:text-gray-2x-light"
                >
                  {{ copied ? 'Copied' : 'Copy' }}
                </button>
              </div>
            ...
            <button
              type="button"
              @click="created = null"
              class="w-full cursor-pointer rounded-xl border-2 border-gray-light py-2.5 font-bold text-gray-x-light transition duration-200 ease-in-out hover:border-gray-x-light hover:text-gray-2x-light"
            >
              Dismiss
            </button>
          </template>
        ...
        <div :class="panelClass">
          ...
          <form class="flex gap-3" @submit.prevent="joinGame">
            ...
            <input
              ...
              placeholder="ABC123"
              class="min-w-0 flex-1 rounded-xl border-2 border-gray-light bg-gray-dark px-4 py-3 text-center text-xl font-bold uppercase tracking-[0.3em] text-gray-2x-light transition duration-200 ease-in-out placeholder:tracking-normal placeholder:text-gray-light hover:border-gray-x-light/60 focus:border-teal-light focus:outline-none"
            />
            <button
              type="submit"
              :disabled="!canJoin || busy"
              class="shrink-0 cursor-pointer rounded-xl border-2 border-gray-light px-6 font-bold text-gray-x-light transition duration-200 ease-in-out hover:border-gray-x-light hover:text-gray-2x-light disabled:cursor-not-allowed disabled:opacity-40"
            >
              Join
            </button>
          </form>
      ...
      <div :class="[panelClass, 'min-h-0']">
        ...
        <div v-if="!hasLoadedMine" class="py-10 text-center text-sm text-gray-x-light">
          Loading your tables…
        </div>

        <div
          v-else-if="!activeGames.length"
          class="flex flex-1 flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-gray-light py-10 text-center"
        >
          <SeatToken :seat-index="-1" size="lg" />
          <div class="flex flex-col gap-0.5">
            <span class="text-sm font-bold uppercase tracking-widest text-gray-x-light"
              >No tables yet</span
            >
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div class="flex min-w-0 flex-1 flex-col gap-1">
                <div class="flex items-center gap-2">
                  <span class="font-bold tracking-[0.25em] text-teal-light">{{
                    game.joinCode
                  }}</span>
                  <span
            ...
            <div class="flex gap-2">
              ...
              <button
                ...
                @click="
                  isConfirming(game.joinCode, 'destroy') ? cancelConfirm() : ask(game, 'destroy')
                "
              >
                {{
                  isConfirming(game.joinCode, 'destroy')
            ...
            >
              ...
              <div
                v-if="eligibleHeirs(game).length > 1"
              ...
              <button
                v-if="chosenHeir(game)"
                type="button"
                :disabled="busy"
                @click="doLeave(game.joinCode, chosenHeir(game).id)"
                class="flex cursor-pointer items-center gap-3 rounded-lg border-2 border-gray-light p-3 text-left transition duration-200 ease-in-out hover:border-teal-light hover:bg-teal-dark/20 disabled:opacity-40"
              >
              ...
              <button
                type="button"
                :disabled="busy"
                @click="doDelete(game.joinCode)"
                class="flex cursor-pointer items-center gap-3 rounded-lg border-2 border-gray-light p-3 text-left transition duration-200 ease-in-out hover:border-rose-400 hover:bg-rose-400/15 disabled:opacity-40"
              >
                <span
                  class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border-2 border-rose-400/50 bg-rose-400/15 text-rose-400"
                  >🗑</span
                >
                <span class="flex min-w-0 flex-col">
            ...
            >
              ...
              <button
                type="button"
                :disabled="busy"
                @click="doLeave(game.joinCode)"
                class="shrink-0 cursor-pointer rounded-lg border-2 border-rose-400 bg-rose-400 px-4 py-2 text-sm font-bold text-gray-dark transition duration-200 ease-in-out hover:bg-rose-300 hover:border-rose-300 disabled:opacity-40"
              >
                {{ guestLabel(game) }}
              </button>
            </div>
...
/*
  Firefox gets the two standard properties; Chromium and Safari need the
  ::-webkit- pseudo-elements. Both are declared because neither covers
  everything yet.
*/
.scroll-slim {
...
/*
  background-clip: content-box plus a transparent border is the trick that
  makes the thumb look inset and pill-shaped: the border reserves padding the
