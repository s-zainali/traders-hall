    ...
    <div class="mx-auto flex w-full max-w-6xl min-h-0 flex-col gap-4 p-6">
        ...
        <header class="grid shrink-0 items-center grid-cols-3 rounded-[1.5rem] border-2 border-gray-light
            ...
            <div class="flex items-center gap-3 justify-self-end">
                ...
                <button type="button" aria-label="Log out" title="Log out" @click="logout" class="flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border-2
                 border-gray-light bg-gray-dark text-gray-x-light transition duration-200 ease-in-out
                 hover:border-rose-400 hover:text-rose-400">⏻</button>
            </div>
        </header>
        ...
        <div class="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
            ...
            <div class="flex min-h-0 flex-col justify-center gap-4">
                ...
                <div :class="panelClass">
                    <div class="flex flex-col gap-1">
                        ...
                        <p class="text-sm text-gray-x-light">Create a table and share the code with up to three others.
                        </p>
                    </div>

                    <button v-if="!created" type="button" :disabled="busy" @click="createGame" class="w-full cursor-pointer rounded-xl border-2 border-teal-light bg-teal-light py-3
                   ...
                   active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-40">
                    ...
                    <template v-else>
                        ...
                        <button type="button" @click="created = null" class="w-full cursor-pointer rounded-xl border-2 border-gray-light py-2.5 font-bold
                     text-gray-x-light transition duration-200 ease-in-out
                     hover:border-gray-x-light hover:text-gray-2x-light">Dismiss</button>
                    </template>
                </div>
            ...
            -->
                        <input v-model="joinCode" maxlength="6" size="6" spellcheck="false" autocapitalize="characters"
                            placeholder="ABC123" class="min-w-0 flex-1 rounded-xl border-2 border-gray-light bg-gray-dark px-4 py-3
                     ...
                     focus:border-teal-light focus:outline-none" />
                        <button type="submit" :disabled="!canJoin || busy" class="shrink-0 cursor-pointer rounded-xl border-2 border-gray-light px-6 font-bold
                     text-gray-x-light transition duration-200 ease-in-out
                     hover:border-gray-x-light hover:text-gray-2x-light
                     disabled:cursor-not-allowed disabled:opacity-40">Join</button>
                    </form>
            ...
            <div :class="[panelClass, 'min-h-0']">
                ...
                <div v-else-if="!activeGames.length" class="flex flex-1 flex-col items-center justify-center gap-3 rounded-2xl border-2
                ...
                <ul v-else class="scroll-slim flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pr-2">
                    <li v-for="game in activeGames" :key="game.id" class="flex flex-col gap-3 rounded-2xl border-2 border-gray-light bg-gray-dark/60 p-4
            ...
            -->
                       ...
                       hover:border-teal-light hover:bg-teal-dark/20 disabled:opacity-40">
                            ...
                            <button type="button" :disabled="busy" @click="doDelete(game.joinCode)" class="flex cursor-pointer items-center gap-3 rounded-lg border-2 border-gray-light
                       p-3 text-left transition duration-200 ease-in-out
                       hover:border-rose-400 hover:bg-rose-400/15 disabled:opacity-40">
                        ...
                        <div v-else-if="isConfirming(game.joinCode, 'destroy')"
                            ...
                            <button type="button" :disabled="busy" @click="doLeave(game.joinCode)" class="shrink-0 cursor-pointer rounded-lg border-2 border-rose-400 bg-rose-400 px-4 py-2
                       text-sm font-bold text-gray-dark transition duration-200 ease-in-out
                       hover:bg-rose-300 hover:border-rose-300 disabled:opacity-40">{{ guestLabel(game) }}</button>
                        </div>
                    </li>
                </ul>
            </div>
