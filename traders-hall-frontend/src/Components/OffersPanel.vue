...
const OFFER_BORDER = {
    sell: {
        base : 'border-rose-400/50', mine: 'border-rose-400/75', claimed: 'border-rose-400'
    },
    trade: {
        base : 'border-amber-400/50', mine: 'border-amber-400/75', claimed: 'border-amber-400'
    },
    rent_out: {
        base : 'border-teal-light/50', mine: 'border-teal-light/75', claimed: 'border-teal-light'
    },
    rent_ask: {
        base : 'border-purple-light/50', mine: 'border-purple-light/75',    claimed: 'border-purple-light'
    },
    invest: {
        base : 'border-blue-light/50', mine: 'border-blue-light/75', claimed: 'border-blue-light'
    }, 
}
    ...
    <div class="flex min-h-0 flex-col rounded-[1.5rem] border-2 border-gray-light bg-gray-x-dark p-4">
        ...
        <ul v-else class="scroll-slim flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pr-1">
            <li v-for="offer in rows" :key="offer.id"
                ...
                :class="twMerge([
                    (OFFER_BORDER[offer.kind].base ?? OFFER_BORDER.trade.base),
                    (offer.claimed
                        ? OFFER_BORDER[offer.kind].claimed
                        : offer.mine
                            ? OFFER_BORDER[offer.kind].mine
                            : ''
                    )])">
        ...
        -->
                <div v-if="offer.isInvest" class="flex items-stretch gap-2">
                    <div
                        class="flex flex-1 flex-col items-center gap-1 rounded-xl border-2 border-gray-light bg-gray-dark/60 px-2 py-2">
                    ...
                    <svg class="h-4 w-4 shrink-0 self-center text-blue-light" viewBox="0 0 16 16" fill="none"
                    ...
                    <div
                        class="flex flex-1 flex-col items-center gap-1 rounded-xl border-2 border-blue-light/40 bg-blue-dark/15 px-2 py-2">
                ...
                <span v-if="offer.isInvest" class="text-[10px] font-bold uppercase tracking-widest text-gray-x-light">
                    for {{ offer.termLabel }}
                </span>
                ...
                <div v-else-if="offer.isRentAsk" class="flex items-center gap-2">
                    ...
                    <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" aria-hidden="true" class="h-4 w-4 shrink-0 text-purple-light">
                        <path d="M2.5 8h11M9.5 4 13.5 8l-4 4" />
                    </svg>
                ...
                <div v-else class="flex items-center gap-2">
                    ...
                    <svg class="h-4 w-4 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"
                        stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" :class="offer.isSell ? 'text-rose-400' : offer.isRent ? 'text-teal-light' : 'text-amber-400'
                            ">
                        <path d="M2.5 8h11M9.5 4 13.5 8l-4 4" />
                    </svg>
                ...
                <div class="flex items-center gap-1.5">
                    <template v-for="(label, i) in STEPS" :key="label">
                        <span class="h-1.5 w-1.5 shrink-0 rounded-full transition-colors duration-300" :class="i <= offer.step ? (i === 1 ? 'bg-amber-400' : 'bg-teal-light') : 'bg-gray-light'
                            "></span>
                    ...
                    <span v-if="offer.claims.length > 1"
                ...
                <div v-if="offer.claimed" class="flex flex-wrap items-center gap-1.5">
                    <button v-for="claim in offer.claims" :key="claim.playerId" type="button"
                        ...
                        class="rounded-lg outline-emerald-400 transition duration-200 ease-in-out" :class="[
                        ...
                        <SeatToken :seat-index="claim.seatIndex" size="sm" :filled="claim.playerId === myPlayerId" />
                    </button>
                </div>

                <!-- Says why Accept is inert. Without a default claimant a disabled
             button with no explanation reads as broken. -->
                <p v-if="offer.mine && !selectedFor(offer)"
                    class="text-[10px] font-bold uppercase tracking-widest text-gray-light">
                ...
                <button v-else type="button" :disabled="busy || !!offer.blocked" @click="emit('claim', offer.id)"
                    class="w-full rounded-xl border-2 py-1.5 text-sm font-bold transition-colors duration-200" :class="offer.blocked
