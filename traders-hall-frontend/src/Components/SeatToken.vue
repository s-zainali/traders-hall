...
<template>
  ...
  >
    <svg :class="glyphs[size]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <!-- crown -->
      <path v-if="seat.token === 'crown'" d="M3 8l4 3 5-7 5 7 4-3-2 11H5L3 8zm2 13h14v2H5v-2z" />
      <!-- diamond -->
      <polygon v-else-if="seat.token === 'diamond'" points="12,2 22,12 12,22 2,12" />
      <!-- shield -->
      <path
        v-else-if="seat.token === 'shield'"
        d="M12 2l8 3.5v6c0 4.8-3.4 8.8-8 10.5-4.6-1.7-8-5.7-8-10.5v-6L12 2z"
      />
      <!-- anchor -->
      <path
        v-else-if="seat.token === 'anchor'"
