...
const KINDS = {
    ...
    parts: (p) => {
      if (isRentKind(p.kind)) {
        return [
          ...
          C(p.card_type, 1),
          T('to'),
          N(p.tenant_player_id ?? p.with_player_id),
          T('for'),
          P(p.rent_points),
          T(`every ${turns(p.rent_interval_turns)}`),
        ]
  ...
  'loan.borrowed': {
    ...
    parts: (p) => [
      T('borrowed'),
      P(p.amount),
      T(`from the bank — due in ${p.due_in_rounds} rounds`),
    ],
  },
  'loan.repaid': {
    icon: 'minusCircle',
  ...
  'mortgage.opened': {
    ...
    parts: (p) => [
      ...
      T('for'),
      P(p.advance),
      T(`— due in ${p.due_in_rounds} rounds`),
    ],
  },
  'mortgage.redeemed': {
    icon: 'unlock',
    tone: 'text-teal-light',
    parts: (p) => [
      T(p.automatic ? 'was charged to redeem' : 'redeemed'),
      C(p.card_type, 1),
      T('for'),
      P(p.amount),
    ],
  ...
  <div class="flex min-h-0 flex-col rounded-[1.5rem] border-2 border-gray-light bg-gray-x-dark p-4">
        ...
        <div v-for="entry in shown" :key="entry.seq" class="flex items-center">
          ...
          >
            ...
            <template v-for="(part, i) in entry.parts" :key="i">
              ...
              <span
                v-else-if="part.t === 'name'"
                :class="toneOf(part.id)"
          ...
          <div class="ml-auto shrink-0 text-xs tabular-nums text-gray-light">
            {{ timeOf(entry.at) }}
          </div>
        </div>
      </template>
    ...
    <form v-if="tab === 'chat'" class="flex shrink-0 gap-2 pt-3" @submit.prevent="send">
      <input
        ...
        class="min-w-0 flex-1 rounded-xl border-2 border-gray-light bg-gray-dark px-4 py-2 text-sm text-gray-2x-light transition duration-200 ease-in-out placeholder:text-gray-light hover:border-gray-x-light/60 focus:border-teal-light focus:outline-none"
      />
      <button
        type="submit"
        :disabled="!draft.trim() || sending"
        class="shrink-0 cursor-pointer rounded-xl border-2 border-teal-light bg-teal-light px-5 text-sm font-bold text-gray-dark transition duration-200 ease-in-out hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
      >
        Send
      </button>
    </form>
