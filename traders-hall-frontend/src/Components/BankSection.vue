...
const railStock = computed(() => ORDER.map((type) => ({ type, count: props.pools[type] ?? 0 })))
...
const hasDebt = computed(() => props.loanOutstanding > 0 || props.mortgageOutstanding > 0)
...
<template>
  ...
  <div class="relative flex h-[calc(100%)] min-h-0 shrink-0 items-stretch">
    ...
    <!--
        ...
        -->
    <div
      class="absolute top-0 right-full bottom-0 z-150 mr-2 w-max transition-all duration-300 ease-out"
      :inert="!expanded"
      ...
      <div class="h-full min-h-0">
        ...
        >
          <TransactionModal
            ...
            :card-type="buyingType"
            :available="pools[buyingType] ?? 1"
            :points="points"
            :busy="busy"
            @confirm="onConfirm"
            @cancel="closeBuy"
          ...
          <BankerModal
            ...
            :loan-due="loanDue"
            :mortgage-card-type="mortgageCardType"
            :mortgage-outstanding="mortgageOutstanding"
            :mortgage-due="mortgageDue"
            :max-loan="maxLoan"
            @close-modal="closeBanker"
            @borrow="(amount) => emit('borrow', amount)"
            @repay="(amount) => emit('repay', amount)"
            @mortgage="(cardType) => emit('mortgage', cardType)"
            @redeem="emit('redeem')"
          />

          <div class="flex shrink-0 justify-between gap-6">
            <div>
              <h1 class="pb-4 text-center text-3xl font-bold tracking-wide text-gray-2x-light">
                Bank
              </h1>
          ...
          <div
            class="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[1rem] border-1 p-4 outline-teal-light transition duration-200 ease-in-out"
            :class="
              buyingActive
                ? 'border-teal-light outline-4 -outline-offset-4 bg-gray-light/30'
                : 'border-gray-light outline-0'
            "
          >
            <button
              v-if="buyingActive"
              @click="emit('cancel')"
              class="absolute top-0 right-0 z-50 flex items-center justify-center p-4 leading-none text-gray-x-light transition duration-200 ease-in-out hover:cursor-pointer hover:text-rose-400"
            >
              🗙
            </button>
            ...
            <h1
              class="shrink-0 pb-4 text-center text-2xl font-bold tracking-wide text-gray-2x-light"
            >
              Cards {{ buyingActive ? '' : '🛈' }}
            </h1>
            ...
            >
              <CardDeck v-for="stock in cardStock" :key="`${stock.type}-${stock.count}`">
                <Card
                  v-for="n in stock.count"
                  :key="`${stock.type}-${n}`"
                  :card-type="stock.type"
                  :buying="buyingActive"
                  @buy="openBuy(stock.type)"
                />
              </CardDeck>
            </div>
    ...
    >
      ...
      <span class="bank-label text-xs font-bold uppercase tracking-[0.3em] text-gray-x-light"
        >Bank</span
      >
