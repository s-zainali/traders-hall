...
const repayClass =
    'bg-teal-light text-gray-dark border-2 border-teal-light hover:brightness-110'
const mortgageClass =
    ...
    <div class="absolute inset-0 z-[100] flex items-center justify-center bg-gray-dark/90 p-4 backdrop-blur-sm"
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="banker-title"
            ...
            <header class="flex items-center gap-3 pr-10">
                <div class="h-10 w-10 shrink-0 bg-teal-light" :style="{
            ...
            <section class="flex shrink-0 flex-col gap-3 border-t-1 border-gray-light pt-5">
                ...
                <template v-if="hasLoan">
                    ...
                    <div class="flex items-center justify-between gap-3">
                        <div :class="[stepperClass, 'shrink-0']">
                            <button type="button" :class="stepButton" :disabled="repayAmount <= 1"
                                aria-label="Decrease repayment" @click="stepRepay(-1)">−</button>
                            <div :class="[countClass, 'w-14 text-lg']">{{ repayAmount }}</div>
                            <button type="button" :class="stepButton" :disabled="repayAmount >= maxRepay"
                                aria-label="Increase repayment" @click="stepRepay(1)">+</button>
                        </div>
                        <button type="button" :class="[actionButton, repayClass, 'shrink-0']"
                            :disabled="locked || !canRepayAnything" @click="emit('repay', repayAmount)">
                            Repay
                        </button>
                    </div>
                </template>
                ...
                <template v-else>
                    ...
                    <div class="flex items-center justify-between gap-3">
                        <div :class="[stepperClass, 'shrink-0']">
                            <button type="button" :class="stepButton" :disabled="borrowAmount <= 1"
                                aria-label="Decrease amount" @click="stepBorrow(-1)">−</button>
                            <div :class="[countClass, 'w-14 text-lg']">{{ borrowAmount }}</div>
                            <button type="button" :class="stepButton" :disabled="borrowAmount >= maxBorrow"
                                aria-label="Increase amount" @click="stepBorrow(1)">+</button>
                        </div>
                        <button type="button" :class="[actionButton, borrowClass, 'shrink-0']" :disabled="locked"
            ...
            <section class="flex shrink-0 flex-col gap-3 border-t-1 border-gray-light pt-5">
                ...
                <template v-if="hasMortgage">
                    <div :class="[wellClass, 'justify-between gap-2 px-4 py-2']">
                        ...
                        <span class="text-2xl font-bold tabular-nums text-rose-400">{{ mortgageOutstanding }}</span>
                    </div>
                    ...
                    <button type="button" :class="[actionButton, repayClass, 'w-full']"
                        :disabled="locked || !canRedeem" @click="emit('redeem')">
                ...
                <template v-else-if="mortgageable.length">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button v-for="option in mortgageable" :key="option.code" type="button"
                            ...
                            :aria-pressed="chosenProperty === option.code" @click="chosenProperty = option.code"
                    ...
                    <button type="button" :class="[actionButton, mortgageClass, 'w-full']"
                        :disabled="locked || !chosenProperty" @click="emit('mortgage', chosenProperty)">
                ...
                <p v-if="mortgageBlocker" class="text-xs font-bold text-gray-light">{{ mortgageBlocker }}</p>
            </section>
