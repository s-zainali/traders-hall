...
const TYPES = {
  ...
  trade: {
    ...
    pointsLabel: '',
    confirmClass:
      'bg-amber-400 text-gray-dark border-2 border-amber-400 hover:bg-amber-300 hover:border-amber-300',
  },
}
...
const tradeableTypes = computed(() =>
  cardTypes.all.filter((c) => c.isTradeable && c.code !== props.cardType).map((c) => c.code),
)
...
<template>
  <div
    :class="
      popover
        ? ''
        : 'absolute inset-0 z-[100] flex items-center justify-center bg-gray-dark/90 backdrop-blur-sm ' +
          (isCompact ? 'p-3' : 'p-6')
    "
    ...
    >
      ...
      <header class="flex flex-col gap-0.5 pr-10">
        ...
        <p class="text-sm text-gray-x-light">
          {{
            toPlayer
              ? 'Name your price per card. Any player can take it.'
              : isTrade
                ? 'Post what you want in return. Any player can take it.'
                : type.subheading
          }}
        </p>
      </header>
      ...
      <div v-if="isTrade" class="flex items-stretch justify-center gap-3">
        <section class="flex w-[10.5rem] flex-col items-center gap-2">
          ...
          <div :class="stepperClass">
            <button
              ...
              :class="stepper"
              :disabled="quantity <= 1"
              aria-label="Decrease quantity given"
              @click="step(-1)"
            >
              −
            </button>
            ...
            <button
              ...
              aria-label="Increase quantity given"
              @click="step(1)"
            >
              +
            </button>
        ...
        <section class="flex w-[10.5rem] flex-col items-center gap-2">
          ...
          >
            <button
              v-for="t in tradeableTypes"
              :key="t"
              type="button"
              :aria-label="`Trade for ${titleOf(t)}`"
              :aria-pressed="getType === t"
              @click="getType = t"
              class="cursor-pointer rounded-xl outline-amber-400 transition duration-200 ease-in-out"
              :class="
                getType === t ? 'outline-3 scale-110' : 'outline-0 opacity-45 hover:opacity-90'
              "
            >
          ...
          <div :class="stepperClass">
            <button
              type="button"
              :class="stepper"
              :disabled="getQuantity <= 1 || !getType"
              aria-label="Decrease quantity wanted"
              @click="stepGet(-1)"
            >
              −
            </button>
            ...
            <button
              ...
              :class="stepper"
              :disabled="!getType"
              aria-label="Increase quantity wanted"
              @click="stepGet(1)"
            >
              +
            </button>
      ...
      <div
        ...
        :class="popover ? 'gap-5' : isCompact ? 'gap-4' : 'gap-6'"
      >
      ...
      <footer class="flex items-center justify-end gap-2 border-t-1 border-gray-light pt-4">
        ...
        <button
          type="button"
          :class="[actionButton, type.confirmClass]"
          :disabled="!canConfirm"
          @click="confirm"
        >
