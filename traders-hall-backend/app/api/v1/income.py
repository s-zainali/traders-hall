...
from app.schemas.income import RollIncomeRequest
from app.services import game_service, income_service
...
_STATUS = {
    ...
    "GAME_FROZEN": status.HTTP_409_CONFLICT,
    "STATE_VERSION_CONFLICT": status.HTTP_409_CONFLICT,
    "ALREADY_ROLLED": status.HTTP_409_CONFLICT,
...
@router.post("/{code}/actions/roll-income", response_model=GameStateOut)
async def roll_income(code: str, body: RollIncomeRequest, user: CurrentUser, db: Db):
    try:
        await income_service.collect(
